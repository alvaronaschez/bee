...
static inline void println_max(int x, int y, char *s, int maxx) {
  if (s == NULL)
    return;
  while (x < maxx && *s != '\0') {
    print_tb(x, y, s);
...
struct text string_to_virtual_lines(const struct string s, int line_len) {
  ...
  while (c) {
    int r;
    char *cc = skip_n_col(c, line_len, &r);
    // TODO: r?? fix skip ncol
    struct string ss;
    string_init(&ss);
    if (cc) {
...
void print_screen(const struct bee *bee) {
  ...
  case INSERT: {
    ...
    struct text t;
    int ins_vlines = 0;
    struct text ins_buf_cpy = get_ins_buf_cpy(bee);
    t = string_to_virtual_lines(ins_buf_cpy.p[0], SCREEN_WIDTH);
    println(MARGIN_LEN, si, t.p[0].p);
    fj = bee->ins_y - 1;
    fi = bee->ins_y + 1;
    si++;
    // text_deinit(&t);
    // text_deinit(&ins_buf_cpy);

    // for(int j = ins_buf_cpy.len -1; j>=0 && sj >= 0; j--){
    //   t = string_to_virtual_lines(ins_buf_cpy.p[j], SCREEN_WIDTH);
    //   if(j==0)
    //     tb_printf(0, sj, MARGIN_FG, MARGIN_BG, "%-3d ", bee->ins_y);
    //   println(MARGIN_LEN, si, t.p[j].p);
    //   //sj --;
    //   si++;
    //   ins_vlines++;
    // }
