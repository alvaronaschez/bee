...
void str_to_lines(const char* s, struct text *out, int screen_width){
  out->len = 0;
  out->p = NULL;

  while(*s!='\0'){
    out->len++;
    out->p = realloc(out->p, out->len*sizeof(char*));

    int len = 0;
    int cap = 8;
    out->p[out->len-1] = malloc(cap);
    for(int i=0; i<screen_width; i+=columnlen(s, i)){
      if(*s == '\0'){
        break;
      }
      if(len + bytelen(s) > cap){
        out->p[out->len-1] = realloc(out->p[out->len-1], cap*=2);
      }
      memcpy(&out->p[out->len-1][len], s, bytelen(s));
      len += bytelen(s);
      s += bytelen(s);
    }
    // insert null termination
    out->p[out->len-1] = realloc(out->p[out->len-1], len+1);
    out->p[out->len-1][len] = '\0';
  }
}
...
void print_screen(const struct bee *bee) {
  ...
  char* s = bee->buf.p[bee->y];
  struct text t;
  str_to_lines(s, &t ,SCREEN_WIDTH);
